...
pub async fn list_monitors_detailed() -> std::result::Result<Vec<SafeMonitor>, MonitorListError> {
    ...
    // Append capture devices to the real displays. If xcap found no monitors
    // (headless / capture-only box) but we have capture devices, surface those
    // instead of erroring so they can still be recorded.
